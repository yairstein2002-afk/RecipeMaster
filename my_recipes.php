...
// --- תיקון: סנכרון סטטוס בזמן אמת מול מסד הנתונים ---
$stmt_status = $pdo->prepare("SELECT status, role FROM users WHERE id = ?");
$stmt_status->execute([$userId]);
$userData = $stmt_status->fetch();

if ($userData) {
    // עדכון הסשן אם חל שינוי בסטטוס (למשל המנהל אישר אותו עכשיו)
    $_SESSION['status'] = $userData['status'];
    $_SESSION['role'] = $userData['role'];
}

$userRole = $_SESSION['role'] ?? 'user';
// מנהל תמיד מאושר, משתמש רגיל לפי הסטטוס ב-DB
...
<head>
    ...
    <style>
        ...
        .filter-group { display: flex; gap: 15px; margin-top: 20px; justify-content: flex-start; }
        ...
        .filter-btn.active { color: white; border-color: var(--accent); background: var(--accent); box-shadow: 0 0 15px rgba(0, 242, 254, 0.4); color: #0f172a; }
...
<div class="container">
    <?php if(empty($my_categories)): ?>
        <div style="text-align:center; padding:100px; opacity:0.5;">
            ...
            <?php if($userStatus === 'approved'): ?>
                <a href="add_recipe.php" style="color:var(--accent); font-size: 1.2rem; font-weight: bold;">לחץ כאן להוספת המתכון הראשון!</a>
            <?php endif; ?>
        </div>
    <?php endif; ?>
    ...
    ?>
        <div class="cat-section" id="cat-<?php echo $cat['id']; ?>">
            ...
            <div class="grid">
                ...
                <div class="card" data-title="<?php echo htmlspecialchars(mb_strtolower($r['title'], 'UTF-8')); ?>" data-status="<?php echo $r['is_public'] ? 'public' : 'private'; ?>">
                    ...
                    <div class="card-actions">
                        ...
                        <a href="delete_recipe.php?id=<?php echo $r['id']; ?>" class="btn-delete" onclick="return confirm('בטוח שרוצה למחוק?')">🗑️ מחיקה</a>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
