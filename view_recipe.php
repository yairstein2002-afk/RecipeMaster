...
if (isset($_POST['report_comment']) && $userId && $userStatus === 'approved') {
    ...
    //הקוד הזה מוודא שדיווח על תגובה לא ילך לאיבוד. במקום לשלוח אותו למנהל אחד, הוא משכפל את ההתראה לכל צוות הניהול, כך שהטיפול יהיה מהיר ויעיל יותר.
...
// זוהי שכבת הגנה שמונעת ממשתמשים בעייתיים או לא מאושרים "ללכלך" את האתר בתגובות, ומפנה אותם חזרה
...
if ($userStatus !== 'approved') {
    ...
    try {
        ...
        $lastCommentId = $pdo->lastInsertId(); 
         // כאן הקוד בודק – אם זו תשובה לתגובה (parentId), היעד להתראה הוא כותב התגובה המקורית. אם זו תגובה ראשית חדשה, היעד הוא בעל המתכון.
        $targetUserId = null;
        if ($parentId) {
...
//מוודא שלא תקבל התראה אם ענית לעצמך או הגבת למתכון של עצמך.
...
//שניגש להציג את התגובות ב-HTML, נוכל בקלות להציג כל "תשובה" בדיוק מתחת לתגובה שהיא קשורה אליה (בדיוק כמו בפייסבוק
$commentsByParent = [];
foreach ($allComments->fetchAll() as $c) { $commentsByParent[$c['parent_id'] ?? 0][] = $c; }
//הפונקציה מחלצת את ה"תעודת זהות" של הסרטון מכל סוג של קישור יוטיוב והופכת אותו לקישור שמתאים להצגה בתוך האתר
...
<script>
    //הפונקציה לוקחת את הכתובת של המתכון הנוכחי, אורזת אותה יפה בתוך קישור שוואטסאפ מבין, ופותחת חלון חדש שמוכן לשליחה לחברים.
...
//זו פונקציה פשוטה אך חזקה שהופכת את האתר שלך לאינטראקטיבי – לחיצה אחת פותחת, לחיצה שנייה סוגרת.
...
//השורה הזו יוצרת הפרדה אישית בין רשימות קניות של משתמשים שונים באותו דפדפן.
const cartKey = 'shopping_list_<?php echo $_SESSION['username'] ?? 'guest'; ?>';

//בכל פעם שאתה לוחץ על מצרך, הפונקציה הזו מעדכנת את הזיכרון של הדפדפן ומעדכנת את המראה של המצרך (הופכת אותו לשקוף או רגיל). זה מאפשר למשתמש לבחור מצרכים ממתכונים שונים ולמצוא את כולם מרוכזים בדף "רשימת הקניות" שלו.
...
// הקוד הזה "משחזר" את הבחירות של המשתמש בכל פעם שהוא נכנס למתכון, כך שחוויית השימוש רציפה והוא לא צריך לזכור מה הוא כבר הוסיף לסל.
