...
?>
<?php
// מניעת שגיאת "Session already active"
...
if (isset($_GET['action'])) {
    // ניקוי כל ההתראות של המשתמש הנוכחי
    if ($_GET['action'] === 'clear_all') {
        $pdo->prepare("DELETE FROM notifications WHERE user_id = ?")->execute([$userId]);
        header("Location: notifications.php?status=cleared"); exit;
    }
    ...
    if ($userRole === 'admin' && isset($_GET['target_id'])) {
        ...
        // מחיקת תגובה + כל הדיווחים עליה + תגובות בת
        if ($_GET['action'] === 'delete_comment') {
            // מחיקת כל ההתראות/דיווחים שקשורים לתגובה הזו (מכל המנהלים)
            $pdo->prepare("DELETE FROM notifications WHERE comment_id = ?")->execute([$targetId]);
            // מחיקת תגובות שהן תשובה לתגובה הזו
            $pdo->prepare("DELETE FROM comments WHERE parent_id = ?")->execute([$targetId]);
            // מחיקת התגובה עצמה
            $pdo->prepare("DELETE FROM comments WHERE id = ?")->execute([$targetId]);
            
        } 
        // חסימת משתמש עם הגנה על מנהלים
        elseif ($_GET['action'] === 'ban') {
            $stmt = $pdo->prepare("SELECT id, role FROM users WHERE id = ?");
            $stmt->execute([$targetId]);
            $target = $stmt->fetch();
            
            // בדיקה שלא חוסמים מנהל או את עצמנו
            if ($target && $target['role'] !== 'admin' && $target['id'] !== $userId) {
                $pdo->prepare("UPDATE users SET status = 'banned' WHERE id = ?")->execute([$targetId]);
                // ניקוי התראות שנוצרו על ידי המשתמש החסום
                $pdo->prepare("DELETE FROM notifications WHERE actor_id = ?")->execute([$targetId]);
            } else {
                header("Location: notifications.php?error=forbidden"); exit;
            }
        }
        header("Location: notifications.php?status=success"); exit;
    }
}

// --- 2. שאילתה לשליפת התראות (כולל פרטי כותב לדיווחים) ---
$sql = "
    SELECT 
        n.id, n.recipe_id, n.created_at, n.actor_id, n.report_reason, n.comment_id, n.is_read,
        u.username as actor_name,
...
<head>
    ...
    <style>
        ...
        .btn-clear { background: rgba(255, 255, 255, 0.05); color: #94a3b8; border: 1px solid rgba(255,255,255,0.1); padding: 8px 16px; border-radius: 50px; text-decoration: none; font-size: 0.85rem; transition: 0.3s; }
        .btn-clear:hover { background: rgba(255, 71, 87, 0.1); color: var(--danger); border-color: var(--danger); }

        .notif-card { background: var(--glass); padding: 20px; border-radius: 15px; margin-bottom: 15px; border: 1px solid rgba(255,255,255,0.05); position: relative; transition: 0.3s; }
        .status-report { border-right: 5px solid var(--danger); background: rgba(255, 71, 87, 0.08); }
        .status-system { border-right: 5px solid var(--success); background: rgba(46, 204, 113, 0.08); }
        ...
        .btn-admin { padding: 6px 14px; border-radius: 6px; font-size: 0.85rem; text-decoration: none; font-weight: bold; cursor: pointer; border: none; transition: 0.2s; }
        .btn-del { border: 1px solid var(--danger); color: var(--danger); background: none; }
        .btn-del:hover { background: var(--danger); color: white; }
        .btn-ban { border: 1px solid var(--warning); color: var(--warning); background: none; }
        .btn-ban:hover { background: var(--warning); color: black; }
        ...
        .system-badge { background: var(--success); color: var(--bg); padding: 2px 8px; border-radius: 4px; font-size: 0.7rem; font-weight: bold; margin-bottom: 8px; display: inline-block; }
    </style>
</head>
<body>
...
<div class="container">
    <div class="header">
        ...
        <div class="nav-actions">
            <?php if(!empty($notifications)): ?>
                <a href="?action=clear_all" class="btn-clear" onclick="return confirm('למחוק את כל ההתראות?')">🧹 נקה הכל</a>
            <?php endif; ?>
    ...
    <?php if (empty($notifications)): ?>
        <div style="text-align: center; padding: 100px 0; opacity: 0.5;">
            <h2>אין התראות חדשות</h2>
            <p>כשמישהו יגיב או יעשה לייק, תראה את זה כאן.</p>
        </div>
    <?php endif; ?>
    ...
    <?php foreach ($notifications as $n): 
        ...
        // זיהוי הודעת מערכת (אישור פרופיל)
        $isSystem = ($n['recipe_id'] == 0 && $n['comment_id'] == 0 && !$isReport);
    ?>
        <div class="notif-card <?php echo $isReport ? 'status-report' : ($isSystem ? 'status-system' : ''); ?>">
            
            <?php if ($isSystem): ?>
                <div class="system-badge">מערכת</div>
                <div style="display: flex; align-items: center; gap: 15px;">
                    <span style="font-size: 1.8rem;">🎉</span>
                    <div>
                        <strong style="color: var(--success); font-size: 1.1rem;">הפרופיל שלך אושר בהצלחה!</strong><br>
                        <span style="font-size: 0.9rem; opacity: 0.8;">עכשיו אתה חלק רשמי מהקהילה. תתחיל לשתף מתכונים!</span>
                    </div>
                </div>

            <?php elseif ($isReport): ?>
                <small style="color: var(--danger); font-weight: bold;">🚩 דיווח על תגובה</small><br>
                <a href="profile.php?id=<?php echo $n['actor_id']; ?>" class="actor-link"><?php echo htmlspecialchars($n['actor_name']); ?></a>
                דיווח על תוכן במתכון: <strong><?php echo htmlspecialchars($n['recipe_title']); ?></strong>
                <div style="background: rgba(255, 71, 87, 0.1); padding: 12px; border-radius: 8px; margin: 10px 0; border: 1px dashed var(--danger); font-size: 0.9rem;">
                    <b>הסיבה שדווחה:</b> <?php echo htmlspecialchars($n['report_reason']); ?>
                </div>
                <div class="comment-box">"<?php echo htmlspecialchars($n['comment_text']); ?>"</div>

                <?php if ($userRole === 'admin'): ?>
                    <div class="admin-actions">
                        <a href="?action=delete_comment&target_id=<?php echo $n['comment_id']; ?>" 
                           class="btn-admin btn-del" 
                           onclick="return confirm('פעולה זו תמחק את התגובה וכל הדיווחים עליה. להמשיך?')">🗑️ מחק תגובה ודיווחים</a>
                        
                        <?php if ($n['author_role'] !== 'admin' && $n['author_id'] !== $userId): ?>
                            <a href="?action=ban&target_id=<?php echo $n['author_id']; ?>" 
                               class="btn-admin btn-ban" 
                               onclick="return confirm('האם אתה בטוח שברצונך לחסום משתמש זה?')">🚫 חסום משתמש</a>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
