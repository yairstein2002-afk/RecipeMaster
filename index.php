...
$pendingRecipesCount = 0;// מספר מתכונים הממתינים לאישור
$pendingUsersCount = 0;// מספר משתמשים הממתינים לאישור (או הרשמות חדשות)
$unreadNotifications = 0; // הודעות מערכת/התראות שטרם נקראו
...
if ($isLoggedIn) {
    // הקוד הזה מכין את הקרקע לבדיקות שראינו קודם. בלי השורה הזו, לשרת אין מושג אם המשתמש שלפניו הוא מנהל, חסום, או סתם אורח.
    $stmt_user = $pdo->prepare("SELECT status, `role`, is_blocked FROM users WHERE id = ?");
    //כאן קורה הביצוע האמיתי והנתונים עוברים מהמסד לתוך ה-PHP שלך.
    $stmt_user->execute([$userId]);
    $dbUser = $stmt_user->fetch();
//המערכת בודקת – "האם מצאנו את המשתמש בבסיס הנתונים?
    if ($dbUser) {
...
<main class="container">
    ...
    <section id="sec-<?php echo $sec['id']; ?>" class="content-section">
        ...
        <div class="recipe-grid" id="grid-<?php echo $sec['id']; ?>">
            ...
            <?php 
                // לוגיקת מניעת שבירה ב-PHP:
                // 1. יצירת קישור למייל (Gravatar) למקרה שאין תמונה
                $email_hash = md5(strtolower(trim($r['email'] ?? '0')));
                $gravatar = "https://www.gravatar.com/avatar/$email_hash?d=mp";
                
                // 2. קביעת התמונה להצגה: אם הקובץ פיזית בשרת - הצג אותו, אחרת - הצג Gravatar
                $userImg = (!empty($r['profile_img']) && file_exists($r['profile_img'])) 
                           ? $r['profile_img'] 
                           : $gravatar;
            ?>
            ...
            <a href="view_recipe.php?id=<?php echo $r['id']; ?>" class="recipe-card <?php echo ($index >= 4) ? 'hidden-item' : ''; ?>" 
                ...
                <div class="img-wrapper">
                    ...
                    <div class="stat-badge">
                        ...
                        <span style="opacity: 0.5;">|</span>
                        <span>💬 <?php echo $r['comments_count'] ?? 0; ?></span>
                    </div>
                </div>
                ...
                <div style="padding: 15px;">
                    ...
                    <div style="display: flex; align-items: center; justify-content: space-between; margin-top: 10px;">
                        <div class="author-link" onclick="event.preventDefault(); window.location.href='profile.php?id=<?php echo $r['user_id']; ?>';">
                            <img src="<?php echo htmlspecialchars($userImg); ?>" 
                                 class="user-avatar-small" 
                                 loading="lazy"
                                 onerror="this.src='<?php echo $gravatar; ?>';">
...
<footer style="margin-top: 60px; padding: 40px 20px; border-top: 1px solid rgba(255,255,255,0.1); background: rgba(15, 23, 42, 0.9); text-align: center;">
        <div style="max-width: 1200px; margin: 0 auto;">
            <div style="margin-bottom: 20px; display: flex; justify-content: center; gap: 25px; flex-wrap: wrap;">
                <a href="accessibility.html" style="color: #00f2fe; text-decoration: none; font-weight: 500; font-size: 0.95rem;">הצהרת נגישות</a>
                <a href="privacy.html" style="color: #00f2fe; text-decoration: none; font-weight: 500; font-size: 0.95rem;">מדיניות פרטיות</a>
                <a href="terms.html" style="color: #00f2fe; text-decoration: none; font-weight: 500; font-size: 0.95rem;">תנאי שימוש</a>
            </div>

            <div style="color: rgba(255,255,255,0.5); font-size: 0.85rem; line-height: 1.6;">
                <p style="margin: 5px 0;">© <?php echo date("Y"); ?> <strong>RecipeMaster</strong> - כל הזכויות שמורות ליאיר שטיין</p>
                <p style="margin: 5px 0; font-size: 0.75rem; opacity: 0.6;">האתר הונגש ובוצעה בדיקה בתאריך 11/03/2026</p>
            </div>
        </div>
    </footer>

    <script src="https://cdn.enable.co.il/v1/enable.js"></script>

    <script>
        // לוגיקה תפעולית של הדף
        ...
        // חיבור ה-ID לצורך ה-Skip Link (דילוג לעיוורים)
