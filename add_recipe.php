...
//הקוד הזה הופך תמונה גולמית וכבדה לקובץ אופטימלי, בעל שם ייחודי, שמוכן להצגה באתר.
            if ($source) {
                $newWidth = 800;
    ...
    // הגדרת הרשאה 0600: רק בעל הקובץ (השרת) יכול לקרוא ולכתוב. 
    // כל שאר המשתמשים בשרת לא יוכלו לראות את תוכן הקובץ.
...
<div class="container">
    ...
    <form method="POST" enctype="multipart/form-data" id="recipeForm">
        ...
        <div id="ing-area">
            <div class="row">
                <input type="text" name="ing_amounts[]" placeholder="כמות (גרם או מ'ל)" style="flex: 1;">
...
//הפונקציה הזו היא "מתג" (Switch) פשוט שמיועד להציג או להסתיר את אזור העזרה בוידאו.
function toggleHelp() {
    const help = document.getElementById('video-help');
    help.style.display = (help.style.display === 'block') ? 'none' : 'block';
}
//הפונקציה הזו יוצרת תצוגה מקדימה מיידית לתמונה שהמשתמש בחר, עוד לפני שהיא נשלחת לשרת.
...
//הפונקציה הזו היא ה"ביטול" (Undo) של בחירת התמונה. היא מחזירה את המצב לקדמותו אם המשתמש התחרט או בחר תמונה לא נכונה.
...
//הפונקציה הזו היא ה"מנוע" שמאפשר למשתמש להוסיף כמה מצרכים שהוא רוצה למתכון שלו באופן דינמי.
function addIng() {
...
//זה הקוד שהופך את כתיבת הוראות ההכנה לפשוטה ונוחה – לוחצים על כפתור, מקבלים מקום לשלב חדש, וממשיכים לבשל.
function addStep() {
